...
export function CheckoutContactPage({
  ...
  useEffect(() => {
    try {
      const savedForm = sessionStorage.getItem('checkout_contact_form')
      if (savedForm) {
        const parsedForm = JSON.parse(savedForm)
        setForm((prev) => ({ ...prev, ...parsedForm }))
        sessionStorage.removeItem('checkout_contact_form')
      }
    } catch (error) {
      console.error('[CheckoutContactPage] Failed to restore form from sessionStorage:', error)
    }
  }, [])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (
      ...
      // Task 1.3: Save form to sessionStorage before navigation
      try {
        sessionStorage.setItem('checkout_contact_form', JSON.stringify(form))
      } catch (error) {
        console.error('[CheckoutContactPage] Failed to save form to sessionStorage:', error)
      }
      onNext(form)
    } else {
  ...
  // Task 1.3: Save form to sessionStorage whenever it changes (for persistence)
  useEffect(() => {
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-300">Your information is saved</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      // Store current form data before sign out
                      sessionStorage.setItem('checkout_contact_form', JSON.stringify(form))
                      onSignOut?.()
                    }}
